...
interface DocumentRealTransitFormProps {
    isSetStatus: boolean,
    initialValues?: any,
    form?: FormInstance,
    onFieldsChange?: any
...
const DocumentRealTransitForm: FC<DocumentRealTransitFormProps> = (props) => {
            ...
            if (!isSetDocumentName && props.initialValues["documentRealIds"]) {
                if (props.initialValues["documentRealIds"].length === 1) {
                    requestToAPI.post("system/documentreal/getname", props.initialValues["documentRealIds"][0])
                        .then((response: any) => {
                            if (props.form) props.form.setFieldsValue({ "documentRealName": response.value });
                    ...
                    if (props.form) props.form.setFieldsValue({ "documentRealName": "Выбрано несколько записей" });
                    setIsSetDocumentName(true);
                }
            }
        }
