...
interface DocumentRealTransitsProps{
    initialValues?: objectWithAnyFields,
    form?: FormInstance,
    documentRealId: number,
    onFieldsChange?: () => void,

}

export const DocumentRealTransits: FC<DocumentRealTransitsProps> = ({...props}) => {
    const firstInputRef = React.useRef<any>(null);
    const [isSetDocumentName, setIsSetDocumentName] = React.useState(false);
    const [form] = Form.useForm();
    React.useEffect(() => {
        if (!isSetDocumentName && props.initialValues && props.initialValues["documentRealId"]) {
            requestToAPI.post("system/documentreal/getname", props.initialValues["documentRealId"])
                .then((response: any) => {
                    props.form && props.form.setFieldsValue({ "documentRealName": response.value });
    ...
    const [tableInterface] = React.useState<any>(Object.assign({}, DEFAULT_TABLE_CONTEXT));
    const [, forceUpdate] = React.useReducer(x => x + 1, 0);
    const [updateRecords, setUpdateRecords] = React.useState([]);
    const documentRealId = props.documentRealId;

    const setFilters = React.useCallback((config: any) => {
