...
const buildFilters = () => {
    return <React.Fragment>
        ...
        <span>События:</span>
        <AuditEventSelect key="kinds" allowClear={true} style={{ width: 380 }}  />
...
const storeFilters = {
    ...
    kinds:{
        transformToSavedValue:(prefval)=>({propInitName:["value"],initValue:prefval})
    },
    idValue:{},
    duration:{},
    proguserId:{
        transformToSavedValue:(prefval)=>({initValue:prefval})
    }
}
            ...
            <AppAffix>
                <div>
                    <ModuleHeader
                        ...
                        buttons={buttons}
                    />
                    <FilterPanelExt filterStored onChange={(fc) => setFilters(fc)} initValues={initFilters} storeFilter={storeFilters}>
                        {buildFilters()}
                    </FilterPanelExt>
                </div>
            </AppAffix>
