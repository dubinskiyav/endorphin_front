...
import CapClassLabelForm from './CapClassLabelForm';
...
const COLUMNS = [
    ...
    {
        title: 'Цвет метки',
        dataIndex: 'capClassValue',
        sorter: true,        
        defaultSortOrder: 'ascend',
        disableQuickFilter: true,        
        render: (capClassValue) => (
            <div style={{ 
              backgroundColor: capClassValue ? `#${capClassValue.toString(16).padStart(6, '0')}` : '#ffffff',
              border: '2px solid #ffffff',
              borderRadius: '15%', 
              boxShadow: '0 0 0 1px #a6a6a6', 
              width: '22px', 
              height: '22px',
              margin: 'auto'              
            }}></div>
...
const buildForm = (form, capClassTypeId) => {
    return <CapClassLabelForm form={form} initialValues={{ capClassTypeId: capClassTypeId }} />
}
...
export default withRouter(CapClassLabel);
